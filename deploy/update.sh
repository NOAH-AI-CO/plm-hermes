...
echo "[2/5] 同步 hermes-config → deploy/hermes-home(扩展 + SOUL + 配置)"
...
echo "[3/5] 同步 PLM SOUL/配置/技能 → 各 profile 实际读取处"
if [ -d skills/plm ]; then
  src=deploy/hermes-home/skills/workspace-skills/plm
  mkdir -p "$src"; rsync -a --delete skills/plm/ "$src/"
  for profile in deploy/hermes-home/profiles/*; do
    [ -d "$profile" ] || continue
    cp hermes-config/SOUL.md "$profile/SOUL.md"
    [ -f hermes-config/config.yaml ] && cp hermes-config/config.yaml "$profile/config.yaml" || true
    mkdir -p "$profile/skills/workspace-skills/plm"
    rsync -a --delete skills/plm/ "$profile/skills/workspace-skills/plm/"
    mkdir -p "$profile/webui/extensions"
    rsync -a --delete hermes-config/webui-extensions/noah/ "$profile/webui/extensions/noah/"
  done
fi
