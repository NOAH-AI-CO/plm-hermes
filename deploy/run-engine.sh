...
set -euo pipefail
cd "$(dirname "$0")/.."
REPO="$(pwd)"; SECRETS="$REPO/deploy/secrets"; RUNTIME="$REPO/deploy/plm-engine-runtime"
...
: "${PLM_BACKEND_BASE:=http://host.docker.internal:8080}"
...
# Docker Desktop cannot overlay individual secret-file bind mounts onto a bound
# source directory. Build a private merged runtime tree instead.
mkdir -p "$RUNTIME"
rsync -a --delete --exclude api.json --exclude setting_test.json --exclude .env --exclude gcp_key.json \
  "$REPO/plm_agent/" "$RUNTIME/"
rm -f "$RUNTIME/api.json" "$RUNTIME/setting_test.json" "$RUNTIME/.env" "$RUNTIME/gcp_key.json"
cp "$SECRETS/api.json" "$RUNTIME/api.json"
cp "$SECRETS/setting_test.json" "$RUNTIME/setting_test.json"
cp "$SECRETS/plm.env" "$RUNTIME/.env"
cp "$SECRETS/gcp_key.json" "$RUNTIME/gcp_key.json"
