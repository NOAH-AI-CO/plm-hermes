...
class ExtractionSchemaOne(BaseModel):
    opinions: List[str] = Field(description="评审意见的提取列表")

class ClassificationSchemaFirstLayer(BaseModel):
    classifications: List[str] = Field(description="分类结果列表，已有维度对应的数字或者新维度的名称，数组长度与输入意见列表一致")
                                     
class ClassificationSchema(BaseModel):
    classifications: List[str] = Field(description="分类结果列表，已有维度对应的数字或者新维度的定义，数组长度与输入意见列表一致")
