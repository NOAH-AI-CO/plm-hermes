# -*- coding: utf-8 -*-
"""
Finance Search V3 HITL Agent.

MindSearchFinanceHitlAgent extends MindSearchAgentV3 with finance-specific tools
and thinking prompt. Coder capability is provided as AgentRunSandbox tool in the
thinking phase (plan 方案 A).
"""
import logging
...
from agent.explore.mindsearch_prompt_v3 import (
    gpt_query_rewrite_user_pt,
    gpt_o_search_final_output_user_pt,
)
from agent.explore.mindsearch_hitl_agent.mindsearch_agent_v3_finance_prompt import (
    finance_thinking_sys_pt, finance_final_output_sys_pt,
)
...
class MindSearchFinanceThinkingAgent(AgentPreset):
    """Thinking agent for Finance Search with stock tools + AgentRunSandbox for computation."""
...
class MindSearchFinanceFinalOutputAgent(AgentPreset):
    llm: BaseLLM = ClaudeHaiku45
    sys_prompt: str = ''
    tools: List[BaseTool] = []


class MindSearchFinanceHitlAgent(MindSearchAgentV3):
    """Finance Search V3 HITL agent. Uses thinking + AgentRunSandbox as tool; no V2 dependency."""
    thinking_agent: MindSearchFinanceThinkingAgent = MindSearchFinanceThinkingAgent()
    final_output_agent: MindSearchFinanceFinalOutputAgent = MindSearchFinanceFinalOutputAgent()
    ...
    def _format_thinking_prompt(
        ...
        return finance_thinking_sys_pt, user_prompt
    ...
    async def _format_final_output_prompt(
        ...
        return finance_final_output_sys_pt, final_user_prompt
