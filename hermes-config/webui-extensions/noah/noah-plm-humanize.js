...
(function () {
    ...
    function labelFor(raw) {
        if (/search-guidelines|检索指南/.test(raw)) return "🔎 正在检索匹配的临床指南…";
    ...
    function schedule() {
        if (scheduled) return;
        scheduled = true;
        requestAnimationFrame(function () { scheduled = false; try { humanizeRows(); } catch (e) {} try { dedupeThinking(); } catch (e) {} });
    }
