...
(function () {
    "use strict";
    // Keep this relative so deployments mounted under /plm-hermes/ reach WebUI.
    var API = "api/plm/admin";
    ...
    function authHeaders(extra){var t=noahToken(),h=Object.assign({},extra||{});if(t)h["Authorization"]="Token "+t;var c=window.__HERMES_CONFIG__&&window.__HERMES_CONFIG__.csrfToken;if(c)h["X-CSRF-Token"]=c;return h;}
    ...
    async function renderGuidelines(pane){
        ...
        async function load(){
            ...
            try{ var d=await jget("/guidelines"); all=(d&&d.guidelines)||[]; draw(); }
        ...
        async function doUnlockFiles(files){
            ...
            for(var i=0;i<files.length;i++){
                ...
                try{
                    var r=await jsend("/unlock","POST",{name:files[i].name,locked:false});
                    if(r&&r.ok) ok++;
    ...
    // 只有 B 端管理员才显示"管理"入口; 客户端 WebUI 在本地鉴权后签发受限 broker 声明。
    function ensureAdmin(cb){
        if(_meChecked){ cb(_isAdmin); return; }
        if(!noahToken()){ cb(false); return; }
        jget("/status").then(function(d){ _isAdmin=!!(d&&d.is_admin); _meChecked=true; cb(_isAdmin); })
                       .catch(function(e){
                           _isAdmin=false;
                           if(e&&((e.status===401)||(e.status===403))) _meChecked=true;
                           cb(false);
                       });   // 未授权时缓存非管理员；网络故障仍可重试
    }
