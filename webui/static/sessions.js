...
async function loadSession(sid){
  ...
  if (currentSid !== sid || forceReload) {
    ...
    if (_msgInner && currentSid !== sid) _msgInner.innerHTML = '<div style="display:flex;align-items:center;justify-content:center;height:100%;color:var(--text-muted);font-size:14px;padding:40px;text-align:center;">加载对话中…</div>';
  }
...
function renderSessionListFromCache(){
  ...
  // PLM 多用户隔离: 隐藏"查看其他账号会话"入口, 每个用户只看到自己的会话。
  const otherProfileCount = 0;
  if(otherProfileCount>0&&!_showAllProfiles){
    ...
    pfToggle.textContent='查看其他账号的 '+otherProfileCount+' 条会话';
