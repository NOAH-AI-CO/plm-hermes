...
function assistantDisplayName(){
  // PLM: 每用户隔离 profile(u_<hash>)是实现细节, 不向用户暴露其 id, 一律显示品牌名。
  if(S.activeProfile&&S.activeProfile!=='default'&&!/^u_[0-9a-f]{16}$/.test(S.activeProfile)) return S.activeProfile.charAt(0).toUpperCase()+S.activeProfile.slice(1);
  return window._botName||'Noah';
}
...
function _applyBusyComposerPlaceholder(){
  ...
  const idlePlaceholder='\u7ed9 '+assistantDisplayName()+' \u53d1\u6d88\u606f\u2026';
