...
function applyBotName(){
  ...
  const msg=$('msg');
  if(msg) msg.placeholder='\u7ed9 '+name+' \u53d1\u6d88\u606f\u2026';
...
(async()=>{
  ...
  try{
    ...
    //
    // The pre-paint inline script writes a default theme before this code
    // runs, but records whether the browser had a prior stored choice. That
    // lets a new client receive the server-side deployment default while an
    // explicit browser choice remains authoritative after a failed autosave.
    ...
    const lsHasExplicitTheme=window.__HERMES_THEME_WAS_STORED__===true&&lsTheme&&['system','light','dark'].includes(lsTheme);
