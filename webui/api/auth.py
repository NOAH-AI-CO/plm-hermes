...
def external_login_url() -> str | None:
    raw = os.getenv('HERMES_WEBUI_LOGIN_URL', '').strip()
    if not raw:
        return None
    parsed = urlsplit(raw)
    if parsed.scheme not in {'http', 'https'} or not parsed.netloc:
        logger.warning('Ignoring invalid HERMES_WEBUI_LOGIN_URL value')
        return None
    return raw
...
def is_auth_enabled() -> bool:
    """True if password auth, passkeys, OIDC, or Django(biz-backend) login is configured."""
    return (
        is_password_auth_enabled()
        or are_passkeys_enabled()
        or is_oidc_auth_enabled()
        or is_django_auth_enabled()
    )
...
def check_auth(handler, parsed) -> bool:
    ...
    if cookie_val and verify_session(cookie_val):
        return True
    # Django(biz-backend) token 通道: SSO 免登后前端携带 noahAccessToken / Authorization
    _dj = _get_django_token_from_request(handler)
    if _dj and verify_django_token(_dj):
        return True
    # Not authorized
...
# ── Django (biz-backend) 登录 / token 校验 胶水 ──────────────────────────────
# 复用 biz-backend 的 DRF Token: 登录打 /api/token/ 换 token, 校验打 /api/users/。
# 供「biz 已登录 → 跳 PLM 免登(SSO handoff)」与「直接访问用 biz 账号登录」共用。
# noahAccessToken cookie 故意非 HttpOnly, 前端 JS 读它带给 plm-engine 做行级归属。
_DJANGO_TOKEN_CACHE: dict = {}          # sha256(token) -> (expires_at, verified, identity)
_DJANGO_TOKEN_INFLIGHT: dict = {}
_DJANGO_TOKEN_CACHE_LOCK = threading.Lock()
_DJANGO_TOKEN_CACHE_MAX = 2048
_DJANGO_TOKEN_CACHE_TTL = 60.0
_DJANGO_TOKEN_NEGATIVE_CACHE_TTL = 10.0


def is_django_auth_enabled() -> bool:
    return bool(os.getenv('HERMES_DJANGO_URL', '').strip())


def get_django_base_url() -> str:
    base = os.getenv('HERMES_DJANGO_URL', '').strip()
    if base:
        return base.split(',')[0].strip().rstrip('/')
    if os.path.exists('/.dockerenv'):
        return 'http://host.docker.internal:8000'
    return 'http://localhost:8000'


def get_django_base_urls() -> list[str]:
    """支持逗号分隔多后端(测试 8101 + 线上 8102): 依次尝试, 谁认就用谁。
    这样两个环境的账号都能登录/校验通过 PLM, 部署一次配好不用切换。"""
    raw = os.getenv('HERMES_DJANGO_URL', '').strip()
    if raw:
        return [b.strip().rstrip('/') for b in raw.split(',') if b.strip()]
    return [get_django_base_url()]


def _django_token_cache_key(token: str) -> str:
    return hashlib.sha256(token.encode('utf-8')).hexdigest()


def _extract_django_user_identity(data) -> str:
    if isinstance(data, dict) and isinstance(data.get('results'), list):
        user = data['results'][0] if data['results'] else {}
...
def _fetch_django_token_identity(token: str) -> tuple[bool, str]:
    import urllib.request
    # 依次试多后端(测试 8101 + 线上 8102), 谁认就用谁。
    for base in get_django_base_urls():
        ...
        try:
            ...
            logger.debug('Failed to verify Django token at %s: %s', base, e)
            continue
        try:
            ident = _extract_django_user_identity(json.loads(raw))
        except Exception:
            continue
        # /api/users/ 对匿名请求也回 200(空列表)——必须拿到真实身份才算验证通过,
        # 否则无 token / 坏 token 会被当成已登录(越权)。
        if ident:
            return True, ident
    return False, ''


def _get_django_token_cache_entry(token: str) -> tuple[bool, str]:
    if not token:
        return False, ''
    now = time.time()
    key = _django_token_cache_key(token)
    with _DJANGO_TOKEN_CACHE_LOCK:
        cached = _DJANGO_TOKEN_CACHE.get(key)
        if cached and cached[0] > now:
            return cached[1], cached[2]
        inflight = _DJANGO_TOKEN_INFLIGHT.get(key)
        if inflight is None:
            inflight = threading.Event()
            _DJANGO_TOKEN_INFLIGHT[key] = inflight
            should_fetch = True
        else:
            should_fetch = False
    if not should_fetch:
        ...
        return False, ''
    try:
        verified, identity = _fetch_django_token_identity(token)
        ttl = _DJANGO_TOKEN_CACHE_TTL if verified else _DJANGO_TOKEN_NEGATIVE_CACHE_TTL
        with _DJANGO_TOKEN_CACHE_LOCK:
            for k in [k for k, v in _DJANGO_TOKEN_CACHE.items() if v[0] <= time.time()]:
                _DJANGO_TOKEN_CACHE.pop(k, None)
            while len(_DJANGO_TOKEN_CACHE) >= _DJANGO_TOKEN_CACHE_MAX:
                _DJANGO_TOKEN_CACHE.pop(next(iter(_DJANGO_TOKEN_CACHE)))
            _DJANGO_TOKEN_CACHE[key] = (time.time() + ttl, verified, identity)
        return verified, identity
    finally:
        with _DJANGO_TOKEN_CACHE_LOCK:
            ev = _DJANGO_TOKEN_INFLIGHT.pop(key, None)
        if ev:
            ev.set()


def verify_django_token(token: str) -> bool:
    return _get_django_token_cache_entry(token)[0]


def resolve_django_user_identity(token: str) -> str:
    return _get_django_token_cache_entry(token)[1]


def _get_django_token_from_request(handler) -> str | None:
    auth = handler.headers.get('Authorization', '') or ''
    parts = auth.split(None, 1)
    if len(parts) == 2 and parts[0].lower() in ('token', 'bearer'):
        return parts[1].strip()
    xh = (handler.headers.get('X-Hermes-User-Token', '') or '').strip()
    if xh:
        return xh
    cookie_header = handler.headers.get('Cookie', '')
    if cookie_header:
        ...
        try:
            ...
            return None
        m = c.get('noahAccessToken')
        if m and m.value:
            return m.value
    return None


def get_django_user_identity_from_request(handler) -> str:
    token = _get_django_token_from_request(handler)
    return resolve_django_user_identity(token) if token else ''


def authenticate_with_django(username, password) -> tuple[str | None, str | None]:
    """打 biz-backend /api/token/ 换 token。依次试多后端(测试+线上), 谁认就用谁。返回 (token, error_msg)。"""
    import urllib.request
    import urllib.error
    ...
    for base in get_django_base_urls():
        ...
        try:
            with urllib.request.urlopen(req, timeout=5) as response:
                if response.status == 200:
                    return json.loads(response.read().decode('utf-8')).get('token'), None
        except urllib.error.HTTPError as he:
            try:
                err = json.loads(he.read().decode('utf-8'))
                if isinstance(err, dict):
                    msg = err.get('detail') or err.get('error')
                    if not msg and err.get('non_field_errors'):
                        msg = err['non_field_errors'][0]
                    last_err = msg or str(err)
                else:
                    last_err = f'Upstream error ({he.code})'
            except Exception:
                last_err = f'Upstream error ({he.code})'
            continue   # 该后端不认(凭证错/无此人), 试下一个
        except Exception as e:
            logger.debug('Failed to authenticate with Django at %s: %s', base, e)
            last_err = f'Upstream communication error: {e}'
            continue
    return None, last_err or 'Unknown authentication failure'


def set_noah_cookies(handler, username: str, token: str) -> None:
    """下发 noahUser + noahAccessToken(非 HttpOnly, 供前端 JS 读取带给 plm-engine)。"""
    ttl = str(_resolve_session_ttl())
    secure = _is_secure_context(handler)
    for name, value in (('noahUser', username or ''), ('noahAccessToken', token or '')):
        ck = http.cookies.SimpleCookie()
        ck[name] = value
        ck[name]['path'] = '/'
        ck[name]['max-age'] = ttl
        ck[name]['samesite'] = 'Lax'
        if secure:
            ck[name]['secure'] = True
        handler.send_header('Set-Cookie', ck[name].OutputString())


def clear_noah_cookies(handler) -> None:
    for name in ('noahUser', 'noahAccessToken'):
        ck = http.cookies.SimpleCookie()
        ck[name] = ''
        ck[name]['path'] = '/'
        ck[name]['max-age'] = '0'
...
def django_profile_for_identity(identity: str) -> str:
    """把 Django 身份映射成稳定的每用户 profile 名(组件 B 会话隔离用)。"""
    return 'u_' + hashlib.sha256(('django:' + (identity or '')).encode()).hexdigest()[:16]


def ensure_user_profile(prof: str) -> bool:
    """确保每用户 profile 存在(克隆 default 配置, 产品 SOUL/skills 随之具备)。
    失败返回 False —— 登录不阻断, 仅回退到默认 profile(不隔离会话, 但功能可用)。"""
    try:
        from api.profiles import get_hermes_home_for_profile, create_profile_api
        home = get_hermes_home_for_profile(prof)
        if home and home.exists():
            return True
        create_profile_api(prof, clone_from='default', clone_config=True)
        return True
    except Exception:
        logger.warning('ensure_user_profile(%s) failed; 回退默认 profile', prof, exc_info=True)
        return False


def set_django_profile_cookie(handler, identity: str, session_cookie_val: str) -> None:
    """登录/SSO 成功后: 把 Django 身份映射成每用户 profile, 下发签名 hermes_profile cookie(会话隔离)。"""
    prof = django_profile_for_identity(identity)
    if not ensure_user_profile(prof):
        return
    try:
        from api.helpers import get_profile_cookie_name
        signed = sign_profile_cookie_value(prof, session_cookie_val)
        ck = http.cookies.SimpleCookie()
        n = get_profile_cookie_name()
        ck[n] = signed
        ck[n]['path'] = '/'
        ck[n]['httponly'] = True
        ck[n]['samesite'] = 'Lax'
        if _is_secure_context(handler):
            ck[n]['secure'] = True
        handler.send_header('Set-Cookie', ck[n].OutputString())
    except Exception:
        logger.warning('set_django_profile_cookie failed', exc_info=True)
