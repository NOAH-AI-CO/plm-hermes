...
_WEBUI_GATEWAY_API_KEY_FILE_ENV = "HERMES_WEBUI_GATEWAY_API_KEY_FILE"
_WEBUI_GATEWAY_USE_RUNS_API_ENV = "HERMES_WEBUI_GATEWAY_USE_RUNS_API"
_GATEWAY_CHAT_BACKENDS = {"gateway", "api_server", "api-server"}
_PLM_GUIDELINE_PREFIX = re.compile(r"^【(?:快速模式|完整报告模式)·指南(NCCN|CSCO|ESMO|CACA)】\s*(.+)", re.S)
_PLM_GUIDELINE_SELECTION = re.compile(r"^我选定第\s*\d+\s*个\s*[:：]", re.S)
_PLM_GUIDELINE_SELECTION_INDEX = re.compile(r"^我选定第\s*(\d+)\s*个\s*[:：]", re.S)
_PLM_GUIDELINES_BLOCK = re.compile(r"```plm-guidelines\s*(.*?)\s*```", re.S)
_PLM_GUIDELINE_SEARCH_URL_ENV = "PLM_GUIDELINE_SEARCH_URL"


def is_plm_guideline_search_turn(message: str) -> bool:
    match = _PLM_GUIDELINE_PREFIX.match(str(message or "").strip())
    return bool(match and not _PLM_GUIDELINE_SELECTION.match(match.group(2).strip()))


def _resolve_selected_doc_id(messages: list, candidate_index: int) -> tuple[str, str] | None:
    """Resolve a card-selection index to its (doc_id, name), from the most
    recent assistant plm-guidelines block that still lists inline candidates."""
    for item in reversed(messages):
        if str(item.get("role") or "") != "assistant":
            continue
        content = str(item.get("content") or "")
        for block in reversed(_PLM_GUIDELINES_BLOCK.findall(content)):
            try:
                candidates = json.loads(block).get("candidates")
            ...
            if not doc_id:
                continue
            return doc_id, str(candidate.get("name") or "").strip()
    return None


def _resolve_original_patient_input(messages: list) -> str | None:
    """Find the patient case description from the first turn of this PLM
    conversation (the message carrying the 【模式·指南X】 prefix, excluding the
    selection turns themselves), so it can be reused without asking the user
    to repeat information they already provided."""
    for item in reversed(messages):
        if str(item.get("role") or "") != "user":
            continue
        prefix_match = _PLM_GUIDELINE_PREFIX.match(str(item.get("content") or "").strip())
        if prefix_match and not _PLM_GUIDELINE_SELECTION.match(prefix_match.group(2).strip()):
            return prefix_match.group(2).strip()
    return None


def _plm_selection_context(message: str, session: Any) -> str | None:
    """Map a card choice to a server-generated guideline candidate doc_id,
    and re-attach the original patient case description, so the downstream
    clarify step doesn't have to ask the user to repeat information they
    already provided in the first turn of this PLM conversation."""
    match = _PLM_GUIDELINE_SELECTION_INDEX.match(str(message or "").strip())
    if not match:
        return None
    candidate_index = int(match.group(1)) - 1
    if candidate_index < 0:
        return None
    messages = list(getattr(session, "context_messages", None) or getattr(session, "messages", None) or [])

    resolved = _resolve_selected_doc_id(messages, candidate_index)
    patient_input = _resolve_original_patient_input(messages)
    if not resolved and not patient_input:
        return None

    parts = ["可信服务上下文："]
    if resolved:
        doc_id, name = resolved
        parts.append(
            "用户在当前候选列表中选定第 "
            f"{candidate_index + 1} 个指南。其 doc_id 是 {doc_id}"
            f"，名称是 {name or '未命名指南'}。必须使用该 doc_id 继续当前 PLM 工作流，不要重新检索或要求用户再次选择。"
        )
    if patient_input:
        parts.append(f"患者病情描述（第①步已提供，直接复用，不要再次向用户询问）：{patient_input}")
    return "".join(parts)


def _plm_guideline_search_request(message: str) -> dict:
    match = _PLM_GUIDELINE_PREFIX.match(str(message or "").strip())
    if not match:
        raise ValueError("PLM guideline prefix is missing")
    organization, patient_input = match.groups()
    url = os.environ.get(
        _PLM_GUIDELINE_SEARCH_URL_ENV,
        "http://plm-engine:8002/plm_evidence_based/select_guideline",
    ).strip()
    headers = {"Content-Type": "application/json"}
    if api_key := _gateway_api_key():
    ...
    request = urllib.request.Request(
        ...
        data=json.dumps({
            ...
            "guideline_priority_order": [organization],
            "compact": False,
        }).encode("utf-8"),
        headers=headers,
        method="POST",
    ...
    if not isinstance(result.get("candidates"), list):
        raise RuntimeError(str(result.get("error") or "No guideline candidates found"))
    return result


def _run_plm_guideline_search_streaming(
    session_id,
    msg_text,
    model,
    workspace,
    ...
    *,
    ...
    """Return PLM guideline cards deterministically for a tagged first turn."""
    ...
    register_active_run(
        ...
        phase="plm-guideline-search",
        workspace=str(workspace),
        model=model,
        provider=model_provider,
        backend="plm-guideline-search",
    )
    ...
    def put(event, data):
        ...
        try:
            ...
            logger.debug("Failed to publish PLM guideline stream event", exc_info=True)

    s = None
    try:
        result = _plm_guideline_search_request(msg_text)
        assistant_text = (
            "请从下方卡片中选择一份。\n\n"
            "```plm-guidelines\n"
            f'{json.dumps({"__plm_guidelines__": True, "candidates": result["candidates"]}, ensure_ascii=False)}\n'
            "```"
        )
        put("token", {"text": assistant_text})
        with _get_session_agent_lock(session_id):
            ...
            assistant_msg = {"role": "assistant", "content": assistant_text, "timestamp": now + 0.000001}
            previous_context = list(getattr(s, "context_messages", None) or getattr(s, "messages", None) or [])
            try:
                from api.streaming import _assign_stable_message_ids, _is_context_compression_marker, _merge_display_messages_after_agent_result

                _assign_stable_message_ids([user_msg, assistant_msg], previous_context, list(getattr(s, "messages", None) or []))
                display_context = [item for item in previous_context if not _is_context_compression_marker(item)]
                s.context_messages = previous_context + [user_msg, assistant_msg]
                display = merge_session_messages_append_only(list(getattr(s, "messages", None) or []), display_context)
                s.messages = _merge_display_messages_after_agent_result(
                    display, previous_context, s.context_messages, str(msg_text or ""), source=pending_source,
                )
            except Exception:
                logger.debug("Failed to merge deterministic PLM guideline turn", exc_info=True)
                s.context_messages = previous_context + [user_msg, assistant_msg]
        ...
        put("done", {"session": redact_session_data(_session_payload_with_full_messages(s, tool_calls=[])), "usage": {}})
        put("stream_end", {"session_id": session_id})
    except Exception as exc:
        logger.warning("PLM guideline search failed", exc_info=True)
        put("apperror", {
            "label": "Guideline search failed",
            "type": "plm_guideline_search_error",
            "message": _redact_text(str(exc))[:500] or "Guideline search failed.",
            "hint": "Check that the PLM guideline engine is available.",
        })
    finally:
        if s is not None:
            try:
                ...
                logger.debug("Failed to clear PLM guideline stream state", exc_info=True)
        with STREAMS_LOCK:
...
def _gateway_api_key(environ: dict[str, str] | None = None) -> str:
    ...
    if configured:
        return configured
    token_file = str(source.get(_WEBUI_GATEWAY_API_KEY_FILE_ENV) or "").strip()
    if not token_file:
        return ""
    try:
        return Path(token_file).read_text(encoding="utf-8").strip()
    except OSError:
        logger.warning("Unable to read %s", _WEBUI_GATEWAY_API_KEY_FILE_ENV)
        return ""
...
def _gateway_session_history(session: Any, *, max_messages: int = 24) -> list[dict]:
    """Return recent user/assistant context suitable for a stateless gateway call."""
    ...
    for entry in session_messages:
        ...
        # Candidate metadata is already summarized in the trusted selection
        # context and can otherwise consume most of the downstream prompt.
        if role == "assistant" and "```plm-guidelines" in str(content):
            continue
...
def _run_gateway_runs_api_streaming(
    ...
    *, put_gateway_event, cancel_event,
    ...
    instructions_parts = []
    conversation_history = _gateway_session_history(session)
    ...
    *,
    ...
    try:
        ...
        try:
            ...
            prefill_messages = []
        selection_context = _plm_selection_context(msg_text, s)
        if selection_context:
            prefill_messages.append({"role": "system", "content": selection_context})
        ...
        if _use_runs_api:
            ...
            if attachments:
                try:
                    ...
                    message_content = str(msg_text or "")
            if selection_context and isinstance(message_content, str):
        ...
        with _get_session_agent_lock(session_id):
            ...
            display_text = getattr(s, "pending_user_message", None) or msg_text
            context_user_msg = {"role": "user", "content": str(msg_text or ""), "timestamp": now}
            display_user_msg = {"role": "user", "content": str(display_text or ""), "timestamp": now}
            pending_source = getattr(s, "pending_user_source", None) or "webui"
            if pending_source != "webui":
                context_user_msg["_source"] = pending_source
                display_user_msg["_source"] = pending_source
            if attachments:
                context_user_msg["attachments"] = list(attachments)
                display_user_msg["attachments"] = list(attachments)
            ...
            try:
                ...
                _assign_stable_message_ids(
                    [context_user_msg, assistant_msg],
                    previous_context,
                    list(getattr(s, "messages", None) or []),
                )
                if context_user_msg.get("id") is not None:
                    display_user_msg["id"] = context_user_msg["id"]
            except Exception:
                logger.debug("Failed to stamp stable ids on gateway turn rows", exc_info=True)
            s.context_messages = previous_context + [context_user_msg, assistant_msg]
            try:
            ...
            try:
                ...
                s.messages = _merge_display_messages_after_agent_result(
                    display,
                    previous_context,
                    [display_user_msg, assistant_msg],
                    str(display_text or ""),
                ...
                if display:
                    ...
                    if isinstance(latest, dict) and latest.get("role") == "user":
                        ...
                        if latest_text == msg_norm:
                            display = display[:-1]
                s.messages = display + [display_user_msg, assistant_msg]
            s.active_stream_id = None
            s.pending_user_message = None
            s.pending_attachments = None
            s.pending_started_at = None
