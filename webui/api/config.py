...
_default_theme = os.getenv("HERMES_WEBUI_DEFAULT_THEME", "dark").strip().lower()
DEFAULT_THEME = _default_theme if _default_theme in {"light", "dark", "system"} else "dark"
...
def print_startup_config() -> None:
    ...
    client_only = os.environ.get("HERMES_WEBUI_CLIENT_ONLY", "").strip().lower() in {
        "1", "true", "yes", "on"
    }
    agent_dir = "not required (client-only)" if client_only else (_AGENT_DIR or "NOT FOUND")
    agent_status = ok if client_only or _AGENT_DIR else err
    ...
    lines = [
        ...
        f"  agent dir   : {agent_dir}  {agent_status}",
        f"  python      : {PYTHON_EXE}",
    ...
    if not _HERMES_FOUND and not client_only:
        print(
...
_SETTINGS_DEFAULTS = {
    ...
    "theme": DEFAULT_THEME,  # light | dark | system
