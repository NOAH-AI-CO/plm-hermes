...
def _is_safe_asset_url(value: str) -> bool:
    ...
    if not any(normalized_path.startswith(prefix) for prefix in _ALLOWED_ASSET_PREFIXES):
        return False

    for prefix in _ALLOWED_ASSET_PREFIXES:
        if normalized_path.startswith(prefix):
            return _is_safe_relative_path(normalized_path[len(prefix) :])
    return False
...
def _manifest_asset_url(value: object, asset_base: str = "") -> str:
    ...
    # bare relative paths resolve under extensions/. Keep this URL relative so
    # index.html's dynamic <base> also supports reverse-proxy subpath mounts.
    # Absolute same-origin paths are still allowed and go through the same
    # validator as env-configured URLs.
    if item.startswith("/"):
        return item
    base = asset_base.strip("/")
    rel = f"{base}/{item}" if base else item
    return EXTENSION_ROUTE_PREFIX.lstrip("/") + rel
