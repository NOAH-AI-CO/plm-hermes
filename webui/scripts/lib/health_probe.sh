...
_hermes_webui_http_get() {
  ...
  if _hermes_webui_truthy "${HERMES_WEBUI_HEALTH_PROBE_PYTHON:-}" && command -v python3 >/dev/null 2>&1; then
    python3 - "${url}" "${max_time}" "${mode}" 2>/dev/null <<'PY'
import ssl
import sys
from urllib.request import urlopen

url, timeout, mode = sys.argv[1:]
context = ssl._create_unverified_context() if mode == "insecure" else None
with urlopen(url, timeout=float(timeout), context=context) as response:
