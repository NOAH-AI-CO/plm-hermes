...
load_secret_file() {
  ...
  if [ -z "$secret_file" ]; then return 0; fi
  if [ ! -r "$secret_file" ]; then error_exit "$file_var is not readable: $secret_file"; fi
  secret_value=$(cat "$secret_file")
  if [ -z "$secret_value" ]; then error_exit "$file_var is empty: $secret_file"; fi
...
if [ "A${whoami}" == "Aroot" ]; then
  ...
  load_secret_file HERMES_API_SERVER_KEY HERMES_API_SERVER_KEY_FILE
  load_secret_file HERMES_LLM_KEY HERMES_LLM_KEY_FILE
